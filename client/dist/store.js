...
import { DebugBarWebSocket, parseHistoryPayload, parseRequestPayload, parseQueryPayload, parseErrorPayload } from './websocket.js';
/**
 * Creates a debug bar store with WebSocket connection
 */
export function createDebugBarStore(config) {
                ...
                case 'request_end': {
                    ...
                    if (request) {
                        state.update((s) => {
                            ...
                            const selectedRequest = s.selectedRequest?.id === request.id ? request : s.selectedRequest;
                ...
                case 'query': {
                    ...
                    if (query) {
                        state.update((s) => {
                            ...
                            const selectedRequest = s.selectedRequest?.id === query.request_id
                                ? {
                                    ...s.selectedRequest,
                ...
                case 'error': {
                    ...
                    if (error) {
                        state.update((s) => {
                            ...
                            const selectedRequest = s.selectedRequest?.id === error.request_id
                                ? {
                                    ...s.selectedRequest,
    ...
    const totalQueries = derived(state, ($state) => $state.requests.reduce((sum, r) => sum + (r.queries?.length || 0), 0));
    const totalErrors = derived(state, ($state) => $state.requests.reduce((sum, r) => sum + (r.errors?.length || 0), 0));
